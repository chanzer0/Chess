...
public class Board960 extends Board {
	public Board960(ChessGUI gui) {
		super(gui);
	}
		...
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if (i == 0 || i == 7) {
					...
					Pawn p = new Pawn(i, j, PlayerEnum.Black, PieceType.Pawn);
					board[i][j].setPiece(p);
				} else if (i == 6) { // if on the 2nd rank, fill with white pawns
					Pawn p = new Pawn(i, j, PlayerEnum.White, PieceType.Pawn);
					board[i][j].setPiece(p);
				}
			}
		}
	...
	private void generateBackRank960() {
		Piece[] backRank = randomBackRank(0, PlayerEnum.Black);
		do {
			backRank = randomBackRank(0, PlayerEnum.Black);
		...
		for (int j = 0; j < backRank.length; j++) {
			...
			pBlack.addRemainingPieces(backRank[j]);
		}

		for (int j = 0; j < backRank.length; j++) {
			if (backRank[j].identifier == PieceType.Rook) {
				board[7][j] = new Tile(7, j, true, new Rook(7, j, PlayerEnum.White, PieceType.Rook));
			}
			if (backRank[j].identifier == PieceType.Bishop) {
				board[7][j] = new Tile(7, j, true, new Bishop(7, j, PlayerEnum.White, PieceType.Bishop));
			}
			if (backRank[j].identifier == PieceType.Knight) {
				board[7][j] = new Tile(7, j, true, new Knight(7, j, PlayerEnum.White, PieceType.Knight));
			}
			if (backRank[j].identifier == PieceType.Queen) {
				board[7][j] = new Tile(7, j, true, new Queen(7, j, PlayerEnum.White, PieceType.Queen));
			}
			if (backRank[j].identifier == PieceType.King) {
				board[7][j] = new Tile(7, j, true, new King(7, j, PlayerEnum.White, PieceType.King));
			}
			pWhite.addRemainingPieces(board[7][j].piece);
		}
		return;
	}
	 ...
	 */
	private Piece[] randomBackRank(int i, PlayerEnum color) {
